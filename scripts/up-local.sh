...
POSTGRES_PORT_DEF=$(read_env POSTGRES_PORT 7432)
...
POSTGRES_PORT=$(pick_free_port "$POSTGRES_PORT_DEF")
...
printf "Using ports -> HTTP:%s BACKEND:%s POSTGRES:%s PGADMIN:%s NEXTCLOUD:%s/%s MAIL:%s/%s\n" \
  "$HTTP_PORT" "$BACKEND_PORT" "$POSTGRES_PORT" "$PGADMIN_PORT" "$NEXTCLOUD_HTTP" "$NEXTCLOUD_API" "$MAILCRAB_UI" "$MAILCRAB_SMTP"

export HTTP_PORT BACKEND_PORT POSTGRES_PORT \
  PGADMIN_PORT NEXTCLOUD_HTTP_PORT NEXTCLOUD_API_WRAPPER_PORT MAILCRAB_FRONTEND_PORT MAILCRAB_SMTP_PORT

# Map our variables to compose expectations where needed
export NEXTCLOUD_API_WRAPPER_PORT="$NEXTCLOUD_API"
export MAILCRAB_FRONTEND_PORT="$MAILCRAB_UI"
export MAILCRAB_SMTP_PORT="$MAILCRAB_SMTP"
